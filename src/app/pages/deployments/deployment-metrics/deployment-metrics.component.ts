...
storageUsageData = [
  ...
  this.filterForm = this.fb.group({
    pods: [this.podList || [], Validators.required],      
    duration: ['15'] ,      
...
renderCpuChart(): void {
  if (!this.chartRef || !this.chartRef.nativeElement) {
    ...
    setTimeout(() => this.renderCpuChart(), 1000);
    return;
  }
  ...
  const labels = this.cpuUsageData.map(item => new Date(item._id));

  // const labels = this.cpuUsageData.map(item =>
  //   new Date(item._id).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
  // );
  const data = this.cpuUsageData.map(item => item.cpuAverage);
  const stepSize = this.filterForm.get('interval')?.value || '5'; 

  if (labels.length === 1) {
    const singleTime = labels[0];
    const offsetTime = new Date(singleTime.getTime() + 60000); // Add 1 minute
    labels.push(offsetTime);
    data.push(data[0]); // Duplicate same value
  }
...
renderRamChart(): void {
  if (!this.chartRef || !this.chartRef.nativeElement) {
    console.warn('chartRef not ready, retrying...');
    setTimeout(() => this.renderRamChart(), 1000);
    return;
  }
  ...
  // const labels = this.ramUsageData.map(item =>
  //   new Date(item._id).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
  // );
  const labels = this.ramUsageData.map(item => new Date(item._id));
  const stepSize = this.filterForm.get('interval')?.value || '5';
  const data = this.ramUsageData.map(item => item.ramAverage);
...
onFilter(): void {
 ...
 // const timeInterval = duration === '15' ? '5' : '15';
  ...
  const timeInterval = formInterval || timeIntervalMap[duration.toString()] || '15';
...
fetchAndAggregateMetrics(pods: string[], fromISO: string, toISO: string, timeInterval: string): void {
  const requests = pods.map(pod => {
    ...
    return this.deploymentService.getDeploymentMetrics(req);
  });
